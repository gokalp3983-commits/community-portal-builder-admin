  ...
  if(req.method === "POST" && url.pathname === "/api/verify-terminal") {
    ...
    req.on("end",async()=>{
      ...
      try{
        ...
        async function probe(pathname,attempts=5){let last=null;for(let attempt=1;attempt<=attempts;attempt++){try{const sep=pathname.includes("?")?"&":"?";const probeUrl=`${base}${pathname}${sep}_cpb_ready=${Date.now()}-${attempt}`;const response=await fetch(probeUrl,{redirect:"follow",signal:AbortSignal.timeout(45_000),headers:{accept:"application/json,text/html","cache-control":"no-cache",pragma:"no-cache"},cache:"no-store"});const text=await response.text();let data=null;try{data=JSON.parse(text)}catch{}const value={status:response.status,ok:response.ok,headers:Object.fromEntries(response.headers),text,data,attempt};const transient=(response.status===404&&value.headers["x-render-routing"]==="no-server")||[502,503,504].includes(response.status);if(!transient)return value;last=value;}catch(error){last={status:0,ok:false,headers:{},text:"",data:null,error:error.message,attempt};}if(attempt<attempts)await new Promise(resolve=>setTimeout(resolve,3000));}return last||{status:0,ok:false,headers:{},text:"",data:null};}
        ...
        const homeLooksRendered=home.status===200&&(/COMMUNITY (?:TERMINAL|PORTAL)/i.test(home.text)||/NFT Collection Terminal|MINT IS LIVE|MINT COUNTDOWN/i.test(home.text));
