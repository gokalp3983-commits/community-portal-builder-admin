...
assert.match(app,/const schedule=nftMintSchedule\(\);const sig=pastScheduleTimeSignature\(schedule\);if\(sig\)\{pastScheduleAcknowledgedSignature=sig;pastScheduleWarningSignature=sig;\}/,"Any dismissal must acknowledge the exact past-schedule signature so one warning never loops");
