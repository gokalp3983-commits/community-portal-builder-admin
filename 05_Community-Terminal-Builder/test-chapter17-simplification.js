...
assert.match(app,/mintScheduleBlock\.addEventListener\("input",\(\)=>invalidateMintConfirmation\(\)\)/,"Phase schedule must validate live on input and invalidate confirmation without reopening the past warning");
assert.match(app,/mintScheduleBlock\.addEventListener\("change",event=>invalidateMintConfirmation\(\{warnPast:mintScheduleTargetChangesTime\(event\.target\)\}\)\)/,"Phase schedule must validate live on change while warning again only for a real time-field change");
