...
(async()=>{
  ...
  const home=await request("/");check(home.status===200,"Builder home returned HTTP 200");const html=await home.text();check(html.includes("COMMUNITY PORTAL BUILDER"),"Builder identity found");check(Boolean(home.headers.get("content-security-policy")),"Content Security Policy present");check(home.headers.get("x-content-type-options")==="nosniff","nosniff header present");
